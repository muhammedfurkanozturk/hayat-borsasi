    ...
    <View style={[styles.container, { backgroundColor: "#ffffff" }]}>
      <View style={styles.headerRow}>
        ...
        <View style={styles.segmentRow}>
          {(["pomodoro", "stopwatch"] as const).map((m) => (
            ...
            >
              <ThemedText style={{ color: mode === m ? DUOLINGO_BLUE : "#71717a", fontSize: 12, fontWeight: "600" }}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 6 }}>
        <Pressable
          ...
          style={[styles.subjectChip, { borderColor: selectedSubjectId === null ? DUOLINGO_BLUE : "#e4e4e7" }]}
        >
          <ThemedText style={{ fontSize: 12, color: selectedSubjectId === null ? DUOLINGO_BLUE : "#3f3f46" }}>Genel</ThemedText>
        </Pressable>
        {subjects.map((s) => (
          <Pressable
            key={s.id}
            ...
            style={[styles.subjectChip, { borderColor: selectedSubjectId === s.id ? DUOLINGO_BLUE : "#e4e4e7" }]}
          >
            <ThemedText style={{ fontSize: 12, color: selectedSubjectId === s.id ? DUOLINGO_BLUE : "#3f3f46" }}>{s.name}</ThemedText>
          </Pressable>
        ))}
      </ScrollView>
      ...
      <View style={{ flexDirection: "row", gap: 10 }}>
        {mode === "pomodoro" ? (
          <>
            ...
            >
              <ThemedText style={{ color: "#71717a", fontSize: 13, fontWeight: "600" }}>Sıfırla</ThemedText>
            </Pressable>
          </>
        ) : (
          <>
            ...
            <Pressable onPress={handleFinishStopwatch} style={styles.secondaryButton}>
              <ThemedText style={{ color: "#71717a", fontSize: 13, fontWeight: "600" }}>Bitir ve Kaydet</ThemedText>
            </Pressable>
          </>
        )}
      </View>
      ...
      <View style={styles.totalRow}>
        <ThemedText style={{ color: "#71717a", fontSize: 12 }}>Bugünkü toplam odaklanma</ThemedText>
      ...
      <View style={styles.barRow}>
        {weekData.map((d, i) => (
          <View key={i} style={styles.barColumn}>
            ...
            <ThemedText style={{ fontSize: 8, color: "#a1a1aa", marginTop: 3 }}>{d.label.slice(0, 2)}</ThemedText>
          </View>
        ))}
      </View>
    </View>
...
function FocusQABox({ sessions, subjects }: { sessions: DbFocusSession[]; subjects: DbFocusSubject[] }) {
  ...
  return (
    <View style={styles.qaBox}>
      <ThemedText style={{ fontSize: 11, fontWeight: "700", color: "#71717a", textTransform: "uppercase", letterSpacing: 0.4 }}>
        Verimine Soru Sor
      </ThemedText>
      ...
      {answer && <ThemedText style={{ fontSize: 13, color: "#27272a" }}>{answer}</ThemedText>}
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  heading: { fontSize: 15, fontWeight: "700", color: "#27272a" },
  segmentRow: { flexDirection: "row", gap: 4, backgroundColor: "#f4f4f5", borderRadius: 8, padding: 3 },
  ...
  subjectInput: { flex: 1, borderWidth: 1, borderColor: "#e4e4e7", borderRadius: 8, paddingHorizontal: 10, height: 38, fontSize: 13, color: "#27272a", backgroundColor: "#fafafa" },
  subjectAddButton: { borderRadius: 8, width: 38, height: 38, alignItems: "center", justifyContent: "center", backgroundColor: "#1cb0f61a" },
  timerBox: { alignItems: "center", justifyContent: "center", paddingVertical: 24, borderWidth: 2, borderColor: "#e4e4e7", borderRadius: 999, marginHorizontal: 40 },
  timerText: { fontSize: 40, fontWeight: "700", color: "#27272a", fontVariant: ["tabular-nums"] },
  primaryButton: { flex: 1, height: 44, borderRadius: 10, alignItems: "center", justifyContent: "center" },
  primaryButtonText: { color: "#ffffff", fontWeight: "700", fontSize: 14 },
  secondaryButton: { flex: 1, height: 44, borderRadius: 10, borderWidth: 1, borderColor: "#e4e4e7", alignItems: "center", justifyContent: "center" },
  totalRow: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", borderTopWidth: 1, borderTopColor: "#f0f0f0", paddingTop: 10 },
  ...
  qaBox: { borderWidth: 1, borderColor: "#e4e4e7", borderRadius: 10, padding: 12, gap: 8 },
  qaInput: { flex: 1, borderWidth: 1, borderColor: "#e4e4e7", borderRadius: 8, paddingHorizontal: 10, height: 38, fontSize: 13, color: "#27272a", backgroundColor: "#fafafa" },
});
