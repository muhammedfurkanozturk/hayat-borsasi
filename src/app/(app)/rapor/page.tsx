...
import { buildCalendarMonthSeries, buildCalendarYearSeries } from "@/lib/chartSeries";
...
import { useAppData, type DailyScorePoint } from "@/lib/supabase/app-data-context";
...
// gerçek kaydı kullanan bir "tarihe göre skor" fonksiyonu üretir — Dashboard'daki
// PeriodIndexCard/ScoreChart ile aynı yaklaşım.
function makeScoreForDate(dailyHistory: DailyScorePoint[], today: string, liveScore: number, pick: (day: DailyScorePoint) => number) {
...
export default function RaporPage() {
  ...
  async function handleGenerate(period: ReportPeriod) {
    ...
    if (period === "Aylık") {
      const values = buildCalendarMonthSeries(overallScoreForDate)
        .map((p) => p.score)
        .filter((s): s is number => s !== null);
      overallScore = average(values);
    } else if (period === "Yıllık") {
      const values = buildCalendarYearSeries(overallScoreForDate)
        .map((p) => p.score)
        .filter((s): s is number => s !== null);
      overallScore = average(values);
    }
    ...
    const categorySummaries = categories.map((category) => {
      ...
      const series = period === "Aylık" ? buildCalendarMonthSeries(categoryScoreForDate) : buildCalendarYearSeries(categoryScoreForDate);
      const values = series.map((p) => p.score).filter((s): s is number => s !== null);
      return { name: category.name, score: average(values) };
    });
  ...
  return (
    <div className="flex flex-1 flex-col">
      ...
      <main className="relative flex w-full flex-1 flex-col gap-6 px-6 py-8 sm:px-10">
        {locked && (
          <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-4 bg-background/60 px-6 py-16 backdrop-blur-sm">
            ...
            <Link
              href="/pro"
              className="flex items-center gap-2 rounded-lg bg-pro px-5 py-2.5 text-sm font-semibold text-pro-foreground hover:opacity-90"
            >
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            {periods.map((p) => (
              <button
                ...
                className="flex items-center justify-center gap-2 rounded-xl border-2 border-border-soft bg-background-elevated px-4 py-4 text-sm font-semibold text-foreground transition-colors hover:border-accent/50 hover:text-accent disabled:pointer-events-none disabled:opacity-50"
              >
                {loadingPeriod === p ? (
                  <>
          ...
          <div className="rounded-2xl border border-accent/30 bg-accent-soft/10 p-5">
            <div className="mb-2 flex items-center justify-between">
              ...
              <button
                ...
                className="rounded-md border border-negative/40 bg-negative-soft px-2.5 py-1 text-xs font-medium text-negative transition-colors hover:border-negative/70 hover:bg-negative/25"
              >
                Kapat
              </button>
            </div>
