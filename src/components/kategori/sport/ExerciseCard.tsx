...
export function ExerciseCard({
  ...
  return (
    <div
      ...
      className={`group relative flex h-20 w-28 shrink-0 cursor-grab flex-col items-center justify-center gap-1 rounded-lg border-2 border-muted/25 bg-background-elevated px-1.5 text-center active:cursor-grabbing ${
        isDragging ? "opacity-50" : ""
      } ${isNewRecord ? "animate-pulse border-pro/60" : ""}`}
    >
      <DumbbellIcon width={16} height={16} className="text-accent" />
      <span className="line-clamp-2 text-[11px] leading-tight text-foreground">{exercise.name}</span>
      ...
      {lastDoneDate && <span className="text-[9px] text-muted">{formatDaysAgo(lastDoneDate, todayIso())}</span>}
      <button
        type="button"
        onClick={(e) => {
          e.stopPropagation();
