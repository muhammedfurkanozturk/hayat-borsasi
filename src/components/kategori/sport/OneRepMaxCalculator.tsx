...
export function OneRepMaxCalculator({ personalRecords }: { personalRecords: Map<string, PersonalRecord> }) {
  ...
  return (
    <div className="flex flex-col gap-4 rounded-lg border border-border bg-surface shadow-card p-5">
      <div>
        <h2 className="text-sm font-medium text-foreground">1RM Hesaplayıcı</h2>
        ...
        <div className="flex flex-wrap gap-1.5">
          {prExercises.map(([name, pr]) => (
            <button
              ...
              className="btn rounded-full border-2 border-muted/25 px-3 py-1 text-[11px] text-muted hover:border-accent/30 hover:text-foreground"
            >
      ...
      <div className="flex gap-3">
        <label className="flex flex-1 flex-col gap-1">
          <span className="text-xs text-muted">Ağırlık (kg)</span>
          <input
            type="number"
            min={1}
            value={weight}
        ...
        <label className="flex flex-1 flex-col gap-1">
          <span className="text-xs text-muted">Tekrar</span>
          <input
            type="number"
            min={1}
            max={20}
      ...
      {valid && epleyResult != null && (
        <div className="flex flex-col gap-3 rounded-lg border-2 border-accent/25 bg-accent-soft/20 p-4">
          <div className="flex items-baseline gap-2">
            <span className="font-mono text-2xl font-semibold tabular-nums text-accent">{Math.round(epleyResult)}</span>
            <span className="text-sm text-muted">kg tahmini 1RM (Epley)</span>
          </div>
          {brzyckiResult != null && !Number.isNaN(brzyckiResult) && (
            <p className="text-xs text-muted">Brzycki formülüne göre: ~{Math.round(brzyckiResult)} kg</p>
          )}

          <div className="flex flex-col gap-1 border-t border-accent/20 pt-3">
            <p className="text-xs font-medium text-muted">Antrenman yüzdeleri</p>
            <div className="grid grid-cols-3 gap-1.5 sm:grid-cols-5">
              {PERCENTAGES.map((pct) => (
                <div key={pct} className="flex flex-col items-center rounded-lg bg-background-elevated px-2 py-1.5">
